// src/components/CdsProductList.js
...
const CdsProductList = ({ filters = { genres: [], colors: [], availability: [] }, onAddToCart }) => {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            ...
            try {
                ...
                queryParams.append('type', 'cd'); // Specify product type for this list
    ...
    if (loading) {
        return <div className="product-list ml-64">Loading CDs...</div>;
    }
    ...
    return (
        <div className="product-list ml-64">
            {products.length === 0 ? (
                <p>No CD products found matching your criteria.</p>
            ) : (
                products.map((product) => (
                    <Card
                        ...
                        bokorFont={bokorFont}
                        productType="cd" // Consistent product type
                    />
                ))
            )}
        </div>
