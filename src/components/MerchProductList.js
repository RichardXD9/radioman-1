// src/components/MerchProductList.js
...
const MerchProductList = ({ filters = { genres: [], colors: [], availability: [] }, onAddToCart }) => {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            ...
            try {
                ...
                queryParams.append('type', 'merch'); // Specify product type for this list
    ...
    if (loading) {
        return <div className="product-list ml-64">Loading Merchandise...</div>;
    }
    ...
    return (
        <div className="product-list ml-64">
            {products.length === 0 ? (
                <p>No merchandise products found matching your criteria.</p>
            ) : (
                products.map((product) => (
                    <Card
                        ...
                        bokorFont={bokorFont}
                        productType="merch" // Consistent product type
                    />
                ))
            )}
        </div>
